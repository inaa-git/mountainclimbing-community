...
export function ScheduleCard({
  ...
  return (
    <article className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition hover:border-emerald-300 sm:p-6">
      ...
      <Link href={`/schedules/${schedule.id}`}>
        <h2 className="mt-4 text-xl font-bold tracking-tight text-zinc-950 hover:text-emerald-800">{schedule.title}</h2>
      </Link>
      <dl className="mt-4 grid gap-2 text-sm text-zinc-700 sm:grid-cols-2">
        ...
        <div><dt className="sr-only">리더</dt><dd>리더 {leaderNickname}</dd></div>
      </dl>
